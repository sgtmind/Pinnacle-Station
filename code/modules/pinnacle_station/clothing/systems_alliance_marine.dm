...
/obj/item/clothing/suit/armor/systems_alliance/marine/Initialize()
	. = ..()
	slowdown_per_slot[slot_wear_suit] = 1.1

/obj/item/clothing/suit/armor/systems_alliance/marine/kinetic_barrier
	var/open = FALSE
...
/obj/item/clothing/suit/armor/systems_alliance/marine/kinetic_barrier/Initialize()
	. = ..()
	if(ispath(power_cell))
		power_cell = new power_cell(src)
		currently_stored_power = power_cell.use(max_stored_power)

/obj/item/clothing/suit/armor/systems_alliance/marine/kinetic_barrier/Destroy()
	QDEL_NULL(shield)
	if(!ispath(power_cell))
...
/obj/item/clothing/suit/armor/systems_alliance/marine/kinetic_barrier/Process(wait)
...
/obj/item/clothing/suit/armor/systems_alliance/marine/examine(mob/user, distance)
	. = ..()
	if(open)
		if(power_cell)
...
/obj/item/clothing/suit/armor/systems_alliance/marine/kinetic_barrier/attackby(var/obj/item/W, var/mob/user)
...
/obj/item/clothing/suit/armor/systems_alliance/marine/kinetic_barrier/attack_self(var/mob/living/user)
	if (open && power_cell)
...
/obj/item/clothing/suit/armor/systems_alliance/marine/kinetic_barrier/attack_hand(var/mob/living/user)
	if(open && (loc == user))
		if(power_cell)
...
/obj/item/clothing/suit/armor/systems_alliance/marine/kinetic_barrier/proc/on_remove_cell()
	power_cell = null
	currently_stored_power = 0
	enable_when_powered = FALSE
	update_icon()

/obj/item/clothing/suit/armor/systems_alliance/marine/kinetic_barrier/dropped(var/mob/user)
	turn_off()
	. = ..()

/obj/item/clothing/suit/armor/systems_alliance/marine/equipped(var/mob/user, var/slot)
	if(slot != SLOT_OCLOTHING && slot != slot_l_hand && slot != slot_r_hand)
		turn_off()
	. = ..()


/obj/item/clothing/suit/armor/systems_alliance/marine/kinetic_barrier/proc/turn_on(var/mob/user)
	enable_when_powered = FALSE
	if(shield || open || !user)
		return
	if(!power_cell)
...
/obj/item/clothing/suit/armor/systems_alliance/marine/kinetic_barrier/proc/turn_off(var/mob/user)
	if(!shield)
		return
	QDEL_NULL(shield)

/obj/item/clothing/suit/armor/systems_alliance/marine/kinetic_barrier/proc/toggle(var/mob/user)
	if(shield)
		turn_off(user)
	else
		turn_on(user)

/obj/item/clothing/suit/armor/systems_alliance/marine/kinetic_barrier/AltClick(mob/user)
	if (loc == user)
		toggle(user)
	else
		. = ..()

/obj/item/clothing/suit/armor/systems_alliance/marine/kinetic_barrier/proc/take_charge()
	if(!actual_take_charge())
		turn_off()
		return FALSE
	return TRUE

/obj/item/clothing/suit/armor/systems_alliance/marine/kinetic_barrier/proc/actual_take_charge()
	if(!power_cell)
		return FALSE
	if(currently_stored_power < shield_power_cost)
		return FALSE
...
/obj/item/clothing/suit/armor/systems_alliance/marine/proc/update_name()
	if(shield)
